...
def main():
    ...
    parser.add_option("-c", action="store", help="Critical threshold in Minutes", default=30,
                      dest="crit")
    ...
    try:
        ...
        for host in host_list:
            status = str(host.runtime.bootTime).split('+')[0]
            test = datetime.strptime(status, "%Y-%m-%d %H:%M:%S.%f").timetuple()
            now = time.time()
            test = time.mktime(test)
            duree =  (int(now) -int(test)) / 60
        perf=" | minutes="+str(duree)



        if duree <= 60:
            print "CRITICAL - Esx has rebooted since : " + str(humanize.naturaltime(duree*60)) + perf
            sys.exit(2)

        else:
            print humanize.naturaltime(duree*60)
            print "OK - Esx Last boot is : " + str(humanize.naturaltime(duree*60)) + perf
            sys.exit(0)
